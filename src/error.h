...
public:
    FileNotOpenError(int fd) : RedBaseError(
            "Invalid file descriptor: " + std::to_string(fd)) {}
};

class FileNotClosedError : public RedBaseError {
public:
    FileNotClosedError(const std::string &filename) : RedBaseError(
            "File is opened: " + filename) {}
};

class FileExistsError : public RedBaseError {
public:
    FileExistsError(const std::string &filename) : RedBaseError(
            "File already exists: " + filename) {}
};

class FileNotFoundError : public RedBaseError {
public:
    FileNotFoundError(const std::string &filename) : RedBaseError(
...
public:
    RecordNotFoundError(int page_no, int slot_no) : RedBaseError(
            "Record not found: (" + std::to_string(page_no) + "," + std::to_string(slot_no) + ")") {}
};

class InvalidRecordSizeError : public RedBaseError {
public:
    InvalidRecordSizeError(int record_size) : RedBaseError(
            "Invalid record size: " + std::to_string(record_size)) {}
};

// IX errors
class InvalidColLengthError : public RedBaseError {
public:
    InvalidColLengthError(int col_len) : RedBaseError(
            "Invalid column length: " + std::to_string(col_len)) {}
};
...
public:
    IndexNotFoundError(const std::string &tab_name, const std::string &col_name) : RedBaseError(
            "Index not found: " + tab_name + '.' + col_name) {}
};

class IndexExistsError : public RedBaseError {
public:
    IndexExistsError(const std::string &tab_name, const std::string &col_name) : RedBaseError(
            "Index already exists: " + tab_name + '.' + col_name) {}
};
...
public:
    IncompatibleTypeError(const std::string &lhs, const std::string &rhs) : RedBaseError(
            "Incompatible type error: lhs " + lhs + ", rhs " + rhs) {}
};

class AmbiguousColumnError : public RedBaseError {
public:
    AmbiguousColumnError(const std::string &col_name) : RedBaseError(
            "Ambiguous column: " + col_name) {}
};
