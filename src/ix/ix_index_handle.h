...
static int ix_compare(InputBuffer a, InputBuffer b, ColType type, int col_len) {
    switch (type) {
        case TYPE_INT: {
            int ia = *(int *) a;
            int ib = *(int *) b;
...
struct IxNodeHandle {
    ...
    Buffer get_key(int key_idx) const { return keys + key_idx * ihdr->col_len; }

    Rid *get_rid(int rid_idx) const { return &rids[rid_idx]; }

    int lower_bound(InputBuffer target) const {
        if (binary_search) {
            int lo = 0, hi = hdr->num_key;
            while (lo < hi) {
                int mid = (lo + hi) / 2;
                Buffer key_addr = get_key(mid);
            ...
            while (key_idx < hdr->num_key) {
                Buffer key_addr = get_key(key_idx);
                if (ix_compare(target, key_addr, ihdr->col_type, ihdr->col_len) <= 0) {
                    break;
                }
                key_idx++;
    ...
    int upper_bound(InputBuffer key) const {
        if (binary_search) {
            int lo = 0, hi = hdr->num_key;
            while (lo < hi) {
                int mid = (lo + hi) / 2;
                Buffer key_slot = get_key(mid);
...
public:
    ...
    void insert_entry(InputBuffer key, const Rid &rid);

    void delete_entry(InputBuffer key, const Rid &rid);

    Rid get_rid(const Iid &iid) const;

    Iid lower_bound(InputBuffer key) const;

    Iid upper_bound(InputBuffer key) const;
